...
class IMAP4ClientProtocol(asyncio.Protocol):
    ...
    async def raw(self, name, *args, untagged_name=None, timeout=None):
        return await self.execute(Command(name, self.new_tag(), *args, untagged_name=untagged_name,
                                          loop=self.loop, timeout=timeout))
...
class IMAP4(object):
    ...
    async def raw(self, name, *args, untagged_name=None):
        return await self.protocol.raw(name, *args, untagged_name=untagged_name, timeout=self.timeout)
